...
def get_file_content(working_directory, file_path):
    try:
        ...
        if not os.path.isfile(file_path):
            return "Error: File not found or is not a regular " + f'file: "{file_path}"'

        with open(file_path) as file:
            contents = file.read(MAX_CHARS)
            if len(contents) > MAX_CHARS:
                contents += (
                    f'[...File "{file_path}" truncated' + f"at {MAX_CHARS} characters]"
                )
            return contents
...
schema_get_files_content = types.FunctionDeclaration(
    ...
    description="Reads and returns the contents of a file in a specified file."
